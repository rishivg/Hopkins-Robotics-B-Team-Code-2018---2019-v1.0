...
public class AutoTest3 extends OpMode {
    ...
    public void encoderDrive(double Lspeed, double Rspeed, double Inches, double timeoutS, double rampup) throws InterruptedException {
        ...
        double     COUNTS_PER_MOTOR_REV    = 1220 ;    //Set for NevRest 20 drive (560). For 40's change to 1120. For 60's 1680
        ...
        newLeftTarget = (goat.front_left.getCurrentPosition() + goat.back_left.getCurrentPosition() )/2 + (int)(Inches * COUNTS_PER_INCH);
        newRightTarget = (goat.front_right.getCurrentPosition() + goat.back_right.getCurrentPosition() )/2 + (int)(Inches * COUNTS_PER_INCH);
        ...
        while ( (runtime.seconds() < timeoutS) &&
                (Math.abs(goat.front_left.getCurrentPosition() + goat.back_left.getCurrentPosition()) /2 < newLeftTarget  && Math.abs(goat.front_right.getCurrentPosition() + goat.back_right.getCurrentPosition())/2 < newRightTarget)) {
            double rem = (Math.abs(goat.front_left.getCurrentPosition()) + Math.abs(goat.back_left.getCurrentPosition())+ Math.abs(goat.front_right.getCurrentPosition()) + Math.abs(goat.back_right.getCurrentPosition()))/4;
            double NLspeed;
            double NRspeed;
            ...
            goat.front_left.setPower(NLspeed);
            goat.back_left.setPower(NLspeed);
            goat.front_right.setPower(NRspeed);
            goat.back_right.setPower(NRspeed);
        }
        // Stop all motion;
        ...
        telemetry.addData("Path2",  "Running at %7d :%7d", goat.front_left.getCurrentPosition(), goat.front_right.getCurrentPosition());
        telemetry.update();

        //setting resetC as a way to check the current encoder values easily........ two front rights?
        double resetC = ((Math.abs(goat.front_left.getCurrentPosition()) + Math.abs(goat.back_left.getCurrentPosition())+ Math.abs(goat.front_right.getCurrentPosition())+Math.abs(goat.front_right.getCurrentPosition())));
        ...
        //keep waiting while the reset is running... two front rights again?
        while (Math.abs(resetC) > 0){
            resetC =  ((Math.abs(goat.front_left.getCurrentPosition()) + Math.abs(goat.back_left.getCurrentPosition()) + Math.abs(goat.front_right.getCurrentPosition())+Math.abs(goat.back_right.getCurrentPosition())));
            //idle();
        }
