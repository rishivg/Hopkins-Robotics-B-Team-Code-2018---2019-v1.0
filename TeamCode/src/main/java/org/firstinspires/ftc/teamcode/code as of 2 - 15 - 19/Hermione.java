...
public class Hermione extends LinearOpMode {
    ...
    public void drive(){
        ...
        //double gyroAngle = 0; //In radiants, proper rotation, yay!!11!!
        double magnitudeMultiplier = 0;
        double modifiedTheta = theta + Math.PI/4;
        ...
        double CurrentPosition = goat.front_left.getCurrentPosition();
        ...
        telemetry.addData("Drive Encoder", CurrentPosition);
    ...
    public void hook() {
        goat.hook.setMode(DcMotor.RunMode.RUN_WITHOUT_ENCODER);

        goat.hook.setPower(gamepad2.left_trigger + (-1 * gamepad2.right_trigger));

        double CurrentPosition = goat.hook.getCurrentPosition();

        telemetry.addData("Hook Value", gamepad2.left_trigger + (-1 * gamepad2.right_trigger));
        telemetry.addData("Hook Position", CurrentPosition);
    }
    ...
    public void latch(){
        if (gamepad2.x){
            ...
            goat.hook.setTargetPosition(21112);

            goat.hook.setMode(DcMotor.RunMode.RUN_TO_POSITION);

            goat.hook.setPower(1);

            while(Math.abs(goat.hook.getCurrentPosition() - (21112)) > 50){
            }
    ...
    public void yikes(){
        if (gamepad2.y){
            ...
            goat.hook.setTargetPosition(29792);

            goat.hook.setMode(DcMotor.RunMode.RUN_TO_POSITION);

            goat.hook.setPower(1);

            while(Math.abs(goat.hook.getCurrentPosition() - (29792)) > 50){
            }
            goat.hook.setPower(0);
...
/*
    public void linSlideForward() {

        goat.linSlide1.setPower(gamepad2.left_trigger);
;
        telemetry.addData("Linear Forward", (gamepad2.left_trigger));

        if (gamepad2.left_trigger > 0 && gamepad2.right_trigger > 0) {

            goat.linSlide1.setPower(0);
            goat.linSlide2.setPower(0);
        }

    }

    public void linSlideBackward() {

        goat.linSlide2.setPower(gamepad2.right_trigger);

        telemetry.addData("Linear Backward", (gamepad2.right_trigger));
    ...
    public void intake() {
        ...
        goat.intake1.setPower(gamepad2.right_stick_y);

        telemetry.addData("Intake", gamepad2.right_stick_y);
    }
...
*/
    ...
    public void chadArm(){
        ...
        goat.hook.setTargetPosition(chadArmPos);
        if(togMap1.b){
            goat.hook.setPower(0);
        }
        else{
        ...
        telemetry.addData("Arm Encoder:", goat.hook.getCurrentPosition());
        telemetry.update();
